...
class VisualRegression {
  ...
  private async setDocumentDirection(direction: 'rtl' | 'ltr'): Promise<void> {
    ...
    await this.page.waitForTimeout(500); // Wait for the page to re-render after changing direction
  }
  ...
  async takeScreenshot(
    ...
    for (const direction of ['rtl', 'ltr'] as const) {
      ...
      expect(
        await elementToTakeScreenShotFrom.screenshot(options)
      ).toMatchSnapshot({
    ...
    // (Firefox and Safari do not support high contrast mode)
